...
export default class Headline extends PureComponent {
    ...
    _queryList = () => {
        let url = Config.requestUrl + Config.informationPage.getDataList + `?channelIds=96&pageNumber=${this.state.page}`;
        console.log(333, url);
    ...
    _jumpToIndustryInfoContent = (id) => {
        ...
        fetch(url, {method: 'GET'}).then(res => res.json()).then(responseText => {
            ...
            if (responseText) {
                this.props.navigation.navigate('IndustryInfoContent', {id,_refresh:this._queryList})
            }
