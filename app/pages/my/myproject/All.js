...
export default class All extends PureComponent {
    ...
    _getAllProjectList = (userInfo) => {
        let url = Config.requestUrl + Config.myProject.projectList + `?mobile=${userInfo.mobile}&channelIds=123&status=6`;
                        ...
                        <View style={{flex: 1,}}>
                            <View style={{
                                ...
                                <Text style={{
                                    ...
                                    color: item.status === 0 ? "#ababab" : item.status === 1 ? "#eea04d" : item.status === 2 ? "#33b661" : "#ed5345"
                                }}>{item.status === 0 ? '草稿' : item.status === 1 ? '待审核' : item.status === -1 ? '未通过' : '已通过'}</Text>
                            </View>
                            <Text style={{
                                marginTop: px2dp(26),
                                textAlign: 'left',
