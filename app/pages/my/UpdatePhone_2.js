...
export default class UpdatePhone_2 extends PureComponent {
    // 默认属性
    // 构造
    constructor(props) {
        super(props);
    ...
    _toUpdatePhone=()=>{
        ...
        this.setState({visible:true},()=>{
            ...
            let url = Config.requestUrl  +  Config.updatePhonePage.step2 +  `?oldMobile=${params.phone}&newMobile=${phone}&captcha=${verifyCode}`;
            console.log(999,url);
            fetch(url,{method:'POST'}).then(res=>res.json()).then(responseText=>{
                ...
                this.setState({visible:false},()=>{
                    if(responseText.code==='200'){
                        Toast.message(responseText.message);
                        DeviceEventEmitter.emit('accountSetSuccess',true);
                        this.props.navigation.goBack('SecuritySettings');
                    }else{
                        Toast.message(responseText.message);
                    }
                })
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            <Text style={{color: '#333', fontSize: moderateScale(14)}}>新手机号</Text>
                            <Input style={{
                                height: px2dp(96),
                                fontSize: moderateScale(15),
                                color: "#222222", backgroundColor: 'transparent', borderWidth: 0, flex: 1,
                            }} value={phone} placeholder={'输入手机号'}  placeholderTextColor={'#999'} onChangeText={(phone) => {
                                this.setState({phone});
                            }}/>
                        </View>
                    </View>
